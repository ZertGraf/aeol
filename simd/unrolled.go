...
func (a *unrolledBackend) Type() BackendType { return Unrolled }

func (a *unrolledBackend) DotProduct(x, y []float32) float32 {
	n := min(len(x), len(y))
	var sum float32
	i := 0
	for ; i+7 < n; i += 8 {
...
func (a *unrolledBackend) DualDotProduct(input, k1, k2 []float32) (float32, float32) {
...
func (a *unrolledBackend) Sum(x []float32) float32 {
	var sum float32
	i := 0
	for ; i+7 < len(x); i += 8 {
...
func (a *unrolledBackend) MultiplyAccumulate(acc, x, y []float32) {
...
func (a *unrolledBackend) ElementwiseSqrt(x []float32) {
	for i := range x {
		x[i] = float32(math.Sqrt(float64(x[i])))
	}
}

func (a *unrolledBackend) ElementwiseMultiply(x, y, z []float32) {
...
func (a *unrolledBackend) ElementwiseAccumulate(x, z []float32) {
	n := min(len(x), len(z))
	i := 0
	for ; i+7 < n; i += 8 {
		z[i] += x[i]
...
func (a *unrolledBackend) ElementwiseMin(x, y, out []float32) {
...
func (a *unrolledBackend) ElementwiseMax(x, y, out []float32) {
	n := min(len(x), min(len(y), len(out)))
	for i := 0; i < n; i++ {
		if x[i] > y[i] {
			out[i] = x[i]
...
func (a *unrolledBackend) PowerSpectrum(re, im, out []float32) {
...
func (a *unrolledBackend) ComplexMultiplyAccumulate(reA, imA, reB, imB, reOut, imOut []float32) {
...
func (a *unrolledBackend) ComplexMultiplyAccumulateStandard(reA, imA, reB, imB, reOut, imOut []float32) {
	n := min(len(reA), min(len(imA), min(len(reB), min(len(imB), min(len(reOut), len(imOut))))))
	for i := 0; i < n; i++ {
		reOut[i] += reA[i]*reB[i] - imA[i]*imB[i]
		imOut[i] += reA[i]*imB[i] + imA[i]*reB[i]
...
func (a *unrolledBackend) ScaledComplexMultiplyAccumulate(reA, imA, reB, imB, reOut, imOut []float32, scale float32) {
	n := min(len(reA), min(len(imA), min(len(reB), min(len(imB), min(len(reOut), len(imOut))))))
	i := 0
	for ; i+7 < n; i += 8 {
...
func (a *unrolledBackend) ConvolveSinc(input []float32, k1, k2 []float64, factor float64) float32 {
	n := min(len(input), min(len(k1), len(k2)))
